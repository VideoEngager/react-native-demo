...
export const InteractionProvider = ({...rest}) => {
  ...
  const findByCode = async (veShortUrl, env) => {
    try {
      const shortCode = veShortUrl.substring(veShortUrl.lastIndexOf('/') + 1);

      return await fetch(
        `https://${env.videoengagerUrl}/api/shorturls/findByCode/${shortCode}`,
  ...
  const processIncomingUrl = async (veShortUrl, env) => {
    try {
      if (
        ![
      ...
      const data = await findByCode(veShortUrl, env);

      if (data?.error) {
        console.log('Error: ', data.error);
        showError(
          'Shorturl Code Error',
          `Endpoint: ${env.videoengagerUrl} Error: ${data.error}`,
        );
        return;
      }
